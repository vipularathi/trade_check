...
for each_server in server_list:
    drop_pattern = rf'dropcopy_({each_server.lower()}|{each_server.upper()}|{each_server.capitalize()})_positions_{today.strftime("%Y%m%d")}_\d{{6}}\.xlsx'  # sample = dropcopy_MAIN_positions_20241107_165710
    ...
    for index, row in grouped_df.iterrows():
        ...
        for each_strike in row['Strike']:
            ...
            #             else:
            #                 i+=1
            #                 print(f'{i}. Sell Value Mismatch: {each_server.upper()}, {symbol}, {expiry.date()}, {opttype}, {each_strike}\ndropcopy_sell_value:{drop_sell_value}, our_sell_value:{op_sell_value}')
            #                 flag = False
            #         else:
            #             i+=1
            #             print(f'{i}. Sell Quantity Mismatch: {each_server.upper()}, {symbol}, {expiry.date()}, {opttype}, {each_strike}\ndropcopy_sell_qty:{drop_sell_qty}, our_sell_qty:{op_sell_qty}')
            #             flag = False
            #     else:
            #         i+=1
            #         print(f'{i}. Buy Value Mismatch: {each_server.upper()}, {symbol}, {expiry.date()}, {opttype}, {each_strike}\ndropcopy_buy_value:{drop_buy_value}, our_buy_value:{op_buy_value}')
            #         flag = False
            # else:
            #     i+=1
            #     print(f'{i}. Buy Quantity Mismatch: {each_server.upper()}, {symbol}, {expiry.date()}, {opttype}, {each_strike}\ndropcopy_buy_qty:{drop_buy_qty}, our_buy_qty:{op_buy_qty}')
            #     flag = False

            if op_sell_qty != drop_sell_qty:
                i += 1
                print(
                    f'{i}. Sell Quantity Mismatch: {each_server.upper()}, {symbol}, {expiry.date()}, {opttype}, {each_strike}\ndropcopy_sell_qty:{drop_sell_qty}, our_sell_qty:{op_sell_qty}\n')
                flag = False

            if op_sell_price != drop_sell_price:
                i += 1
                print(
                    f'{i}. Sell Price Mismatch: {each_server.upper()}, {symbol}, {expiry.date()}, {opttype}, {each_strike}\ndropcopy_sell_price:{drop_sell_price}, our_sell_price:{op_sell_price}\n')
                flag = False
            ...
            if op_buy_price!= drop_buy_price:
                i += 1
                print(
                    f'{i}. Buy Price Mismatch: {each_server.upper()}, {symbol}, {expiry.date()}, {opttype}, {each_strike}\ndropcopy_buy_price:{drop_buy_price}, our_buy_price:{op_buy_price}\n')
                flag = False

            if op_buy_qty != drop_buy_qty:
                i += 1
                print(
                    f'{i}. Buy Quantity Mismatch: {each_server.upper()}, {symbol}, {expiry.date()}, {opttype}, {each_strike}\ndropcopy_buy_qty:{drop_buy_qty}, our_buy_qty:{op_buy_qty}\n')
                flag = False
